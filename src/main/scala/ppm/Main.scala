...
class Main extends Application {
  ...
  override def start(stage: Stage): Unit = {
    ...
    var octree = createOcTree(MAX_SCALE, worldRoot)
  ...
  def containsAShape(node: Node, root: Group): Boolean = {
    ...
    val shapesList = getAllShapesFromRoot(root)
  ...
  def getContainedShapes(node: Node): List[Shape3D] = {
    ...
    //TODO - deve ser enviada a lista como argumento
  ...
  def createOcTree(scale: Double, root: Group): Octree[Placement] = {
    val placement: Placement = ((0, 0, 0), scale)
    createOcNode(placement, root)
  }
  ...
  def createOcNode(placement: Placement, root: Group): Octree[Placement] = {
    val xyz = placement._1
    val size = placement._2 / 2
    val nodeBox = createShapeCube(placement)
    if (containsAShape(nodeBox, root)) {

      worldRoot.getChildren.add(nodeBox)
      if (placement._2 > MIN_SCALE) {
        OcNode(placement,
          createOcNode(((xyz._1, xyz._2 + size, xyz._3), (size)), root),
          createOcNode(((xyz._1 + size, xyz._2 + size, xyz._3), (size)), root),
          createOcNode(((xyz._1, xyz._2 + size, xyz._3 + size), (size)), root),
          createOcNode(((xyz._1 + size, xyz._2 + size, xyz._3 + size), (size)), root),
          createOcNode((xyz, (size)), root),
          createOcNode(((xyz._1 + size, xyz._2, xyz._3), (size)), root),
          createOcNode(((xyz._1, xyz._2, xyz._3 + size), (size)), root),
          createOcNode(((xyz._1 + size, xyz._2, xyz._3 + size), (size)), root)
        )
      }
      else {
        val newSection: Section = (placement, getContainedShapes(nodeBox))
        OcLeaf(newSection)
      }
  ...
  //TODO - método para desenhar a OcTree
  def drawOcTree(oct: Octree[Placement]): Unit = {
